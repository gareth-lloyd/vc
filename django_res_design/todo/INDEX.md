...
| [BUG-024](bug-024-anonymize-collides-on-multiple-phones.md) | **`Person.anonymize()` cannot erase anyone holding two phone numbers** — every phone is blanked to `""`, so the second row violates `unique_contact_phone` | ⬜ filed 2026-09-02, found while building GAP-101's `anonymised_person` scenario; the email loop already uses a per-row sentinel (`redacted-{pk}@anonymized.local`) and never collides — phones just need the same treatment; `@transaction.atomic` means the failure is clean, but it is total: a mobile plus a landline is enough and the erasure request simply errors |
