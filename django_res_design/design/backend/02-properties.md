...
- ~~`VillaMaster.WebsiteDescription` → `section=OVERVIEW`~~ — the column is actually `VillaMaster.OverView` (`ResSystem/Database/Data/VillaMaster.cs`); loaded as `OVERVIEW` until 2026-09-22, when the section was retired and its 8 rows dropped (see `section` above)
