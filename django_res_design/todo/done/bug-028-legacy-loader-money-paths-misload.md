...
- `lookups.py:81` `is_active = not bool(row.get("DeletedAt"))` is dead code:
  ...
  When fixing, select `DeletedAt` + `DeletedBy` and use GAP-107's
  `legacy_row_deleted(row)` (`loaders/_util.py`, the OR predicate both
  legacy conventions satisfy); **retire, never skip** deleted currencies
  (plans and bookings may reference them), and add a `Currency (active)`
  parity check in `reconcile_legacy` on GAP-107's `Country (active)`
  pattern. *(Folded in from GAP-107's planned follow-up ticket at merge,
  2026-09-14.)*
