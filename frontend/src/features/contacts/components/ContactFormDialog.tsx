...
export function ContactFormDialog(props: ContactFormDialogProps) {
  ...
  // The CompanyPicker works with a Company OBJECT, but the form field `agency`
  // is the org's PK. Hold the selected object for display/rehydration and mirror
  // its id into RHF on every change. Seeded from `agency_detail` in edit mode.
  const [selectedAgency, setSelectedAgency] = useState<Company | null>(
  ...
  const selectAgency = (company: Company) => {
    ...
    form.setValue("agency", company.id, { shouldValidate: true });
  };

  // The agency FK is nullable; the picker only ever emits a company, so detaching
  // one needs its own affordance (parity with the old clearable free-text field).
  const clearAgency = () => {
    setSelectedAgency(null);
    form.setValue("agency", null, { shouldValidate: true });
  };
  ...
  useEffect(() => {
    if (open) {
      ...
      setSelectedAgency(isCreate ? null : (props.contact.agency_detail ?? null));
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4" noValidate>
            ...
            <div className="space-y-2">
              <Label htmlFor="contact-agency">{t("fields.agency")}</Label>
      ...
      {createCompanyOpen ? (
        <CompanyFormDialog
          ...
          onCreated={(company) => selectAgency(company)}
        />
      ) : null}
    </>
  );
