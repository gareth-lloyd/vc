...
The cutover is a one-shot: `loadlegacy --all` runs once, into a fresh,
migrated DB (BUG-029). A late write or a failed run means dropping the DB and
reloading from a newer dump — never an in-place second pass.
"""
