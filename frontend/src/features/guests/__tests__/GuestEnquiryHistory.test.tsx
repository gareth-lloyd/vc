...
import { GuestEnquiryHistory } from "../components/GuestEnquiryHistory";
...
describe("GuestEnquiryHistory", () => {
  it("is collapsed by default and reveals rows on expand", async () => {
    server.use(http.get("/api/v1/guests/55/enquiries", () => HttpResponse.json(HISTORY)));

    renderWithProviders(<GuestEnquiryHistory guestId={55} />);
  ...
  it("shows the empty state when the guest has no enquiries", async () => {
    server.use(
      http.get("/api/v1/guests/77/enquiries", () =>
        HttpResponse.json({ count: 0, next: null, previous: null, results: [] }),
      ),
    );

    renderWithProviders(<GuestEnquiryHistory guestId={77} />);
    await userEvent.click(await screen.findByRole("button", { name: /toggle enquiry history/i }));
  ...
  it("surfaces a 'more not shown' hint when the page is truncated", async () => {
    server.use(
      http.get("/api/v1/guests/55/enquiries", () =>
        HttpResponse.json({
          count: 5,
          next: "http://x/next",
          previous: null,
