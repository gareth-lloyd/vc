 ...
 *
 * Neutral home (GAP-072) so the clients list can read the tag set without an
 * edge into contacts. The backend accepts/returns these exact `value`s (PATCH
 * replaces the whole set; unknown values are rejected with a 400). The UI
 * iterates this array for a stable display order; `labelKey` resolves to a
 * `contacts` i18n key.
 *
