...
def _write_seed(directory: Path, key: str, *, subject: str, mjml: str = "") -> None:
    # Plaintext is derived from the HTML at send time, so seeds carry only the
    # subject and the MJML body — no `.body.txt`.
    (directory / f"{key}.subject.txt").write_text(subject)
    if mjml:
        (directory / f"{key}.body.mjml").write_text(mjml)


def test_discover_seeds_reads_subject_title_and_mjml(tmp_path: Path) -> None:
    _write_seed(tmp_path, "test.seed.simple", subject="Your code\n", mjml=SIMPLE_MJML)
    seeds = discover_seeds(tmp_path)

    assert len(seeds) == 1
    assert seeds[0].key == "test.seed.simple"
    assert seeds[0].title == "Test Seed Simple"
...
def test_sync_creates_missing_templates(tmp_path: Path) -> None:
    _write_seed(tmp_path, "test.seed.simple", subject="Code", mjml=SIMPLE_MJML)
    ...
    assert template.title == "Test Seed Simple"
...
def test_sync_bumps_version_when_content_changes(tmp_path: Path) -> None:
    ...
    updated_mjml = SIMPLE_MJML.replace("Hello {{ name }}", "Updated {{ name }}")
    _write_seed(tmp_path, "test.seed.simple", subject="Code", mjml=updated_mjml)
    result = sync_templates(tmp_path)
    ...
    versions = list(
        ...
        .order_by("version")
        .values("version", "is_active", "body_template_mjml")
    )
    assert versions == [
        {"version": 1, "is_active": False, "body_template_mjml": SIMPLE_MJML},
        {"version": 2, "is_active": True, "body_template_mjml": updated_mjml},
    ]
...
def test_sync_aborts_on_invalid_mjml_with_no_partial_write(tmp_path: Path) -> None:
    _write_seed(tmp_path, "test.seed.simple", subject="Code", mjml=SIMPLE_MJML)
    _write_seed(tmp_path, "test.seed.broken", subject="Bad", mjml="<not-mjml/>")
...
def test_sync_handles_template_without_mjml(tmp_path: Path) -> None:
    _write_seed(tmp_path, "test.seed.plainonly", subject="Plain")
