...
def test_rateperiod_no_overlap_same_plan(plan: RatePlan) -> None:
    ...
    RatePeriod.objects.create(plan=plan, date_from=date(2026, 6, 1), date_to=date(2026, 6, 30))
    with pytest.raises(IntegrityError), transaction.atomic():
        RatePeriod.objects.create(plan=plan, date_from=date(2026, 6, 15), date_to=date(2026, 7, 15))
...
def test_rateperiod_no_overlap_is_inclusive_on_boundaries(plan: RatePlan) -> None:
    ...
    RatePeriod.objects.create(plan=plan, date_from=date(2026, 6, 1), date_to=date(2026, 6, 30))
    with pytest.raises(IntegrityError), transaction.atomic():
        RatePeriod.objects.create(plan=plan, date_from=date(2026, 6, 30), date_to=date(2026, 7, 31))
...
def test_rateperiod_overlap_allowed_across_plans() -> None:
    ...
    RatePeriod.objects.create(plan=plan_a, date_from=date(2026, 6, 1), date_to=date(2026, 6, 30))
    other = RatePeriod.objects.create(
        plan=plan_b, date_from=date(2026, 6, 1), date_to=date(2026, 6, 30)
    )
