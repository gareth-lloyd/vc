...
@dataclass
class SampleGraph:
    """The synthetic records to push, in dependency order (so every nested
    RES_ID resolves against a record already sent), plus media to reap."""

    push_plan: list[tuple[str, list[models.Model]]] = field(default_factory=list)
...
class Command(BaseCommand):
    help = (
        ...
        "payload attribute, push each record to the ZOHO_SAMPLE_WEBHOOK_* "
        "endpoints through the production pipeline, then roll the data back."
    )
        ...
        try:
            ...
            with override_settings(ZOHO_FLOW_WEBHOOKS=sample_urls):
                with transaction.atomic():
                    # Suppress auto-enqueue while building — the send/accept/
                    # create service calls would otherwise queue their own
                    # (real) pushes; we push explicitly and deliberately below.
                    with suppress_zoho_push():
                        graph = self._build_graph()
                    media_paths = graph.media_paths

                    for kind, objects in graph.push_plan:
                        if kind not in kinds:
                            continue
                        if dry_run:
                            self._dry_run_kind(kind, objects)
                        else:
                            self._push_kind(kind, objects, sample_urls[kind])
    ...
    def _push_kind(self, kind: str, objects: list[models.Model], url: str) -> None:
        if not url:
            self.stdout.write(f"[{kind}] sample webhook URL unset — skipped")
            return
        for obj in objects:
    ...
    def _dry_run_kind(self, kind: str, objects: list[models.Model]) -> None:
        for obj in objects:
            spec = get_zoho_spec(obj._meta.model)
            assert spec is not None  # every pushed model is registered
            payload = spec.build_payload(obj)
            self.stdout.write(f"[{kind}] pk={obj.pk} payload:")
            self.stdout.write(json.dumps(payload, indent=2, default=str))

    # ── synthetic graph ──────────────────────────────────────────────────

    def _build_graph(self) -> SampleGraph:
        """Construct the whole synthetic object graph via the app factories +
        production service layer. Returns records grouped by kind in dependency
        order, plus the media file paths to reap after rollback."""
        ...
        # PropertyImage writes a media file that outlives the DB rollback —
        # capture its storage path so the `finally` can reap it.
        media_paths: list[str] = []
        for img in villa.images.all():
            name = img.image.name
            if name:
                media_paths.append(name)

        graph = SampleGraph(media_paths=media_paths)
        graph.push_plan = [
            ("contact", [p1, p2]),  # both persons → both relationship directions
            ("villa", [villa]),
            ("enquiry", [e1, e2]),
            ("quote", [quotation]),
            ("booking", [booking]),
