...
| Personal-data removal under GDPR                                                           | **anonymization-in-place**: an explicit service method (`Person.anonymize()`) overwrites PII fields with sentinels (`"[REDACTED]"`, empty strings) and sets `status=ANONYMIZED`. The row stays so FK integrity on historical bookings is preserved; the fact of anonymization is **visible**, not hidden. Email is replaced on each `PersonEmail` child with `"redacted-{id}@anonymized.local"` (child-table column is `NOT NULL`) — see `people-model-cleanup.md` |
| Merging duplicate records                                                                  | explicit service method (`Person.merge(target)`) — rewrites every FK pointing at `self` to point at `target` (via `queryset.update()`, which **bypasses the audit signals**), then **hard-deletes** `self`. The rewrites are not individually audited; instead the per-relation counts and the destination pk are summarised onto the deletion `AuditLog` row as `__merged_into__` + `__rewrites__` (`core.audit.record_merge`, FG-016). No `merged_into` self-FK                                                                                                              |
...
A single table with a generic foreign key that captures sensitive-field edits as an append-only diff stream. Used in particular by `properties.PropertyFinance` and its children (`Commission`, `TaxPolicy`, `BankAccount`, `PaymentSchedule`, `SecurityDepositPolicy`) and by `accounts.Person` for PII-relevant fields.
...
- `citext` extension for case-insensitive email indices (`PersonEmail.email`).
