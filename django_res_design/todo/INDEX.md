...
| [GAP-073](gap-073-legacy-loader-bulletproofing-reconcile.md) | Legacy-loader bullet-proofing — land the remainder + reconcile the branch against post-GAP-070/065 main | 🟨 partial (2026-07-06) — big hardening effort verified green on `feat/legacy-loader` (standards docs, 6 defects, Zoho/Temenos, availability loader, 4 product decisions). **Role-code fix landed on `main`** (`_ROLE_MAP` keyed on Code not Id — ~197/335 assignments were flattening to Owner). Rest can't merge: branch base `b3abc0e` predates GAP-070 (deleted groups) + GAP-065 (room location) → conflicts + migration collisions; owner steer needed on salvage-only vs reconciliation rebase |
