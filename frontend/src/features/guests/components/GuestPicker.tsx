...
import { useSearchGuests } from "../hooks";
import type { Guest } from "../schemas";
import { guestDisplayName } from "../display";
...
export function GuestPicker({ value, onChange, onCreateNew, disabled }: GuestPickerProps) {
  ...
  const query = useSearchGuests(debouncedSearch);
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          disabled={disabled}
        >
          {value ? guestDisplayName(value) : t("placeholders.picker_trigger")}
        </Button>
      </PopoverTrigger>
                ...
                <li key={g.id}>
                  ...
                  >
                    ...
                    {g.email ? (
                      ...
                      <span className="text-muted-foreground ml-2 text-xs">{g.phone}</span>
                    ) : null}
                  </button>
                </li>
              ))}
