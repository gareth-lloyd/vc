...
class TestSearchOptionsEndpoint:
    ...
    def test_query_count_does_not_scale_per_stay_option(
        ...
        # once per entry, shared between the bounds clamp and the quote (13
        # queries for one entry today, incl. the GAP-037 service lookup). Pin so
        # a per-option or per-block query can't creep in unnoticed.
        _sat_changeover(property_)
        api_client.force_authenticate(staff)
