...
| `APPROVED` | `execute` | `EXECUTING` | user with `payments.refund.execute` perm; **may be `approved_by`** (single op can both authorise the refund and push it to the gateway in low-risk flows; for high-risk/large refunds, an org policy may require `executed_by != approved_by`, enforced in the service, not the model); **+ a fresh TOTP step-up** (GAP-057 — see note below) | create `Payment(purpose=REFUND, status=PROCESSING)` linked via `meta.refund_id`, queue Celery task `process_refund(refund_id)` to call the gateway |
...
- **Refund-execution TOTP step-up (GAP-057).** `:execute` is the money-out click, so it demands a valid, single-use TOTP on **every** call — even when the session already completed a 2FA login (a hijacked cookie alone must not move money). `RefundService.execute(refund, *, actor, tfa_code=None)` performs the check **after** the perm/SoD guards and **below** the idempotency short-circuit (so an idempotent retry of an already-`EXECUTING`/terminal refund never re-demands a code), gated on `settings.TFA_ENFORCED and actor is not None` (the `actor=None` system sentinel is exempt; approve/reject/cancel do **not** step up). The view threads `request.data.get("tfa_code")` through — no serializer change for one optional field. Freshness = the TOTP window (±1 step) plus the `verify_code` replay guard (`User.tfa_last_verified_step`), so a code already consumed by the login challenge in the same 30s window is rejected until the authenticator rolls; recovery codes do **not** work for step-up. Errors flow through the canonical handler: `TfaStepUpRequired` (`DomainError`, `403`, `code="tfa_stepup_required"`) for a missing code or an actor not enrolled at all, `InvalidTfaCode` (`DomainValidationError`, `400`, `code="invalid_tfa_code"`) for a wrong/replayed code — the `403`-vs-`400` split lets the FE tell "need a code" from "that code was wrong, retry". Success emits `logger.info("refund.stepup_verified", refund_id=…, actor_id=…)` (never the code). **The step-up gate is on `TFA_ENFORCED`, not always-on** (deviates from the ticket's belt-and-braces wording): prod/staging always enforce (middleware guarantees every staff actor is enrolled), while dev/test/`seed_dev` stay code-free.
