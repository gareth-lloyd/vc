...
|---|---|---|---|
| 39 | Idempotency-Key generic surface | **GENERIC TABLE + MIDDLEWARE** — add `core.IdempotencyRecord(user, path, key, request_hash, response_status, response_body, response_headers, expires_at)` and `core.middleware.IdempotencyMiddleware`. Drop the per-model `idempotency_key` columns from `Payment`, `Refund`, `SecurityDeposit`. Dedupe lives at the API boundary. Endpoint-policy-driven: payment/booking/refund-creation views set `idempotency_required=True` (header required → `400` if missing); other unsafe POSTs are opt-in. Hash mismatch → `409 Conflict`. Nightly cleanup task. | **Reversed by FG-005 (2026-07-02)** — never built; table dropped, dedupe is service-layer meta-key (see decision log) |
...
| 2026-07-02 | #39 | REVERSED by FG-005. The generic `core.IdempotencyRecord` + `IdempotencyMiddleware` were never implemented; the table (created in `core.0001`, zero runtime writers ever) is dropped in `core.0006`. Live idempotency is the service-layer `idempotency_key` kwarg with `core/idempotency.py` meta-key stamping (`find_by_meta_key` / `stamp_meta`) plus per-model DB backstops (FG-010); prefer a natural key where one exists. | Dedupe moved from the API boundary into the services that actually needed it (refund / manual-payment creation, owner-block import). A middleware covering "every unsafe POST" solved a problem no caller had, at the cost of a response-caching table; the per-service pattern shipped incrementally and is the documented convention (`django_res/CLAUDE.md` §"State-mutating services accept `idempotency_key`"). |
