...
11. **Legacy import** (subsumes the former GAP-012 "image binary migration"
    tracker) — `properties/management/commands/import_legacy_images.py`
    ...
    **Nested-source → flat-target reconstruction (load-bearing).** The legacy
    .NET app stores files **nested under the integer villa id**, not flat:
    ...
    `properties/legacy/<filename>` key — so the Django row alone cannot locate
    its source file. Rebuild the source path from each row:

    | Need | Source |
    |---|---|
    | target id / key | `PropertyImage.image` = `properties/legacy/<filename>` (flat) |
    ...
    So the copy is `PropertyImages/<property.legacy_id>/<filename>` →
    CF custom id `properties/legacy/<filename>`. Worked example (2026-06-09):
    ...
    **Flatten-collision check (re-run per dump).** Flattening into one
    `properties/legacy/` namespace is only safe if filenames are globally unique
    across villas. Verified 2026-06-09: **12,293 rows, 12,293 distinct keys, 0
    collisions** (legacy `Name` values are GUIDs). This is a property of the
    data, not a guarantee — re-verify on the cutover dump before importing; a
    single colliding `Name` across two villas would make two rows resolve to one
    image, silently overwriting.

    **Source files are not in the repo/container** — `res-app:/app/wwwroot/
    PropertyImages` is empty and the path is gitignored; ops must export the
    archived legacy tree (S3 / backup tarball) for `--source` (see Prereq 2).
    Treat **missing-at-source** as the expected-loss bucket: log
    copied / missing-at-source / collision counts, don't crash the run. A
    verification pass should confirm every `properties/legacy/` row resolves to
    a CF image (and list any that don't).
