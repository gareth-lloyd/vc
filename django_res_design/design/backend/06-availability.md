...
|---|---|---|
| `modify_dates(date_from, date_to, *, actor, reason)` | AWAITING_DEPOSIT, DEPOSIT_PAID, AWAITING_BALANCE, BALANCE_PAID | Locks the row, re-runs `PricingEngine.quote(...)` against the new range (`allow_projection=False`), then replaces `pricing_snapshot` with the engine breakdown re-netted by `BookingService.reprice_snapshot` (`gross` kept, the line's operator discount re-applied as the same amount floored at 0, BUG-025) and sets `rental_price` / `balance_due` from it. Overlap with another blocking booking is refused by the `booking_no_overlap_blocking` exclusion constraint (`OverlappingBooking`, fields rolled back); no `BookingHold` is taken and `balance_due_at` is not recomputed. Fires `booking_total_changed` so the unsettled schedule resizes. Writes a `BookingEvent` with `meta={"from": [date_from, date_to], "to": [...], "from_snapshot": {...}, "to_snapshot": {...}}`. Refused once `status == CHECKED_IN` (use `cancel` + re-book instead) or from terminal states. |
| `modify_guests(adults, children, *, actor, reason)` | every state up to and including BALANCE_PAID (not from CHECKED_IN or terminal states) | Updates party-size fields; re-runs `PricingEngine.quote(...)` because party size can resolve to a different `RateRule` (occupancy band) and re-derives `rental_price` / `balance_due` through the same `reprice_snapshot` netting as `modify_dates`, then fires `booking_total_changed`. Writes a `BookingEvent` with `meta={"from": {...}, "to": {...}, "from_snapshot": {...}, "to_snapshot": {...}}`. |
