...
| [BUG-026](bug-026-no-deposit-booking-parked-in-awaiting-deposit.md) | **A booking that wants no deposit is parked in `awaiting_deposit` forever** — nothing advances that status except a settled/waived DEPOSIT row, so a booking with no deposit row never moves and its balance settle is swallowed as an invalid transition | 🔴 filed 2026-09-03 while shipping BUG-022/023; **pre-existing**, reachable via `deposit_required=False` at confirmation, a zero override at confirmation (pinned by `test_create_for_booking__zero_override_creates_no_deposit_row`) and now a zero override / policy flip on resync; `expire_bookings` needs a PENDING deposit so it cannot reap these either; fix = a `skip_deposit()` transition when the schedule leaves no active deposit, or let `record_balance` admit `AWAITING_DEPOSIT` when none exists |
