...
Backed by `properties.PropertyDescription` (per-property × per-section child rows; see `02-properties.md`). Sections are a fixed enum: `overview`, `house-rules`, `villa-info`, `further-info`. Sections are sparse — a property may have zero, one, or all four rows. *As built (2026-09-22):* `house-rules`, `web-des-1`, `web-des-2`, `interior-sub`, `interior-para`, `exterior-sub`, `exterior-para`, `location-sub`, `location-para`, `internal-notes`, `other-information`, `rooms` — one section per legacy column, each website block a sub/para pair; `further-info`, `location` and `web-description` are gone (GAP-090) as are `villa-info` (GAP-091) and `overview` (retired 2026-09-22, migration `properties/0011`; unknown slugs 404). `other-information` is edited on the Features tab next to the tags; everything else, `rooms` included (GAP-092), on the property's Descriptions tab. `PUT` upserts (creates or replaces); `DELETE` removes the row (server returns empty body for the section). The flat columns the legacy `VillaMaster` carried (`WebsiteDescription`, `HouseRules`, `FeatureDescription`, `RoomDescription`) are migrated into rows of this child table — see reconciliation issue #28.
